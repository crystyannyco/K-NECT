...
<div class="max-w-md mx-auto p-8 mt-12 bg-white shadow-2xl rounded-2xl border border-blue-100 animate-fade-in">
    ...
    <form action="<?= base_url('admin/categories/edit/' . $category['id']) ?>" method="post" class="space-y-8">
        ...
        <div>
            ...
            <input type="text" name="name" value="<?= esc($category['name']) ?>" class="w-full border border-blue-200 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-400 focus:outline-none shadow-sm transition" required />
        </div>
