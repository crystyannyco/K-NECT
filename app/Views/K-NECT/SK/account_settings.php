		...
		<div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-5 lg:p-6 mb-4 sm:mb-5 lg:mb-6">
			...
			<div class="p-6">
				<div id="profile-tab" class="tab-content" role="tabpanel" aria-labelledby="tab-profile">
								<form action="<?= base_url('sk/account-settings/profile') ?>" method="post" enctype="multipart/form-data">
									<?= csrf_field() ?>
				...
				<div id="security-tab" class="tab-content hidden" role="tabpanel" aria-labelledby="tab-security">
					<div class="mb-8 pb-8 border-b border-gray-200">
						<h3 class="text-lg font-semibold mb-4">Change Password</h3>
									<form id="password-form" action="<?= base_url('sk/account-settings/password') ?>" method="post">
										<?= csrf_field() ?>
							<div class="space-y-4 max-w-md">
								<div>
									<label class="block text-sm font-medium text-gray-700 mb-1">Current Password</label>
												<input type="password" name="current_password" class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm">
								</div>
								<div>
									<label for="new_password" class="block text-sm font-medium text-gray-700 mb-1">New Password</label>
									<input type="password" id="new_password" name="sk_password" class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm">
								...
								<div>
									...
									<input type="password" id="confirm_password" name="confirm_password" class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm">
								...
								<div class="flex justify-end">
									<button type="submit" id="pwdSubmitButton" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-md">Update Password</button>
								</div>
							</div>
						</form>
					</div>
...
(function() {
	...
	buttons.forEach(btn => {
		btn.addEventListener('click', (e) => {
			e.preventDefault();
			activate(btn);
			// Persist selected tab in URL hash without scroll
...
// Ensure the global header doesn't duplicate these flash toasts
...
if (input) {
	input.addEventListener('change', () => {
		...
		if (!file) return;
		// Basic validation to mirror server: image only, <= 2MB
...
if (pwdForm) {
	pwdForm.addEventListener('submit', function(e) {
		...
		const nw = pwdForm.querySelector('input[name="new_password"]');
		...
		// Live strength and match checks like KK
	...
	(function initAccountSettingsValidation(){
		...
		const EMAIL_CHECK_URL = '<?= base_url('sk/account-settings/check-email') ?>'; // optional; will fail-open if missing route
		const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/i;
		let emailTimer;
		async function isEmailAvailable(email){
			// Skip check if unchanged or invalid
		...
		function validateEmail(){
			...
			// Tentative clear before async uniqueness
		...
		function normalizePhone(raw){
			...
			if (digits.startsWith('0')) digits = digits.slice(1); // reject leading 0
		...
		form.addEventListener('submit', async (e)=>{
			// Missing required fields guard (email, phone)
			const missing = [];
