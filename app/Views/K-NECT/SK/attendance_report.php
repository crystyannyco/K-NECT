...
async function downloadAttendancePDF() {
    ...
    try {
        ...
        const skLogoPath = (logos.sk?.file_path) || (logos.barangay?.file_path) || '';
        const irigaLogoPath = (logos.iriga_city?.file_path) || '';
        const skLogoUrl = skLogoPath ? '<?= base_url() ?>' + skLogoPath : '';
        ...
        const [skLogoDataUrl, irigaLogoDataUrl] = await Promise.all([
            imageUrlToDataUrl(skLogoUrl),
        ...
        generateAttendancePDFWithLogos(doc, skLogoDataUrl, irigaLogoDataUrl, button, originalHTML);
...
function generateAttendancePDFWithLogos(doc, skLogoDataUrl, irigaLogoDataUrl, button, originalHTML) {
    try {
        let yPosition = 15;
        ...
        if (skLogoDataUrl) {
            doc.addImage(skLogoDataUrl, getImgFmt(skLogoDataUrl), leftLogoX, yPosition, logoSize, logoSize, undefined, 'FAST');
        }
        if (irigaLogoDataUrl) {
    ...
    doc.autoTable({
        ...
        // Save and download
        const fileName = `SK_${eventData.title.replace(/[^a-zA-Z0-9]/g, '_')}_Attendance_${new Date().toISOString().split('T')[0]}.pdf`;
        doc.save(fileName);
